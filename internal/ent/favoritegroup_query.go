...
//		GroupBy(favoritegroup.FieldCreatedAt).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.FavoriteGroup.Query().
//		Select(favoritegroup.FieldCreatedAt).
//		Scan(ctx, &v)
...
func (_q *FavoriteGroupQuery) loadFavorites(ctx context.Context, query *FavoriteQuery, nodes []*FavoriteGroup, init func(*FavoriteGroup), assign func(*FavoriteGroup, *Favorite)) error {
	...
	nodeids := make(map[int]*FavoriteGroup)
	for i := range nodes {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(favorite.FieldGroupID)
	}
	query.Where(predicate.Favorite(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(favoritegroup.FavoritesColumn), fks...))
	}))
